...
export const AuthModal = ({ isOpen = true, onClose, initialMode = 'signup' }) => {
  ...
  return (
    <div className="auth-modal-overlay-backdrop fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md overflow-y-auto">
      <div className="auth-modal-content-card bg-surface border border-border rounded-card relative w-full max-w-2xl p-6 sm:p-8 text-text-primary shadow-card my-8 max-h-[92vh] overflow-y-auto">
        {/* Modal Top Control Bar */}
        <div className="modal-top-control-bar flex items-center justify-between pb-3">
          <button
            type="button"
            onClick={handleClose}
            className="modal-back-nav-btn flex items-center gap-1.5 text-xs text-text-secondary hover:text-white transition-colors cursor-pointer"
          >
          ...
          <button
            type="button"
            onClick={handleClose}
            className="modal-close-icon-btn w-8 h-8 rounded-full bg-surface-raised hover:bg-border flex items-center justify-center text-text-secondary hover:text-white transition-all cursor-pointer"
            aria-label="Close"
          >
            <X className="w-4 h-4" />
          </button>
