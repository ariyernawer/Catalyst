...
export default function DiscoverPage() {
  ...
  competitionApi
    ...
    .then((res) => {
      ...
      setBookmarkedIds(new Set(items.map((event) => event.id ?? event._id)));
    })
    .catch(() => {
      // not signed in or fetch failed, bookmarks just stay empty.
    });
...
const competitionsWithBookmarks = useMemo(
  () => competitions.map((c) => ({ ...c, isBookmarked: bookmarkedIds.has(c.id) })),
  ...
  return competitionsWithBookmarks.filter((competition) => {
  ...
  }, [competitionsWithBookmarks, filters, searchQuery]);  
  ...
  const wasBookmarked = bookmarkedIds.has(competition.id);

  // Flip the heart immediately, revert if the API fails.
  setBookmarkedIds((current) => {
    const next = new Set(current);
    if (wasBookmarked) next.delete(competition.id);
    else next.add(competition.id);
    return next;
  });

  try {
    if (wasBookmarked) await competitionApi.removeBookmark(competition.id);
    else await competitionApi.toggleBookmark(competition.id);
  } catch {
    setBookmarkedIds((current) => {
      const next = new Set(current);
      if (wasBookmarked) next.add(competition.id);
      else next.delete(competition.id);
      return next;
    });
  }
};
  ...
  const savedCompetitions = competitionsWithBookmarks.filter((c) => c.isBookmarked);
    ...
    >
      <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <label className="block">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateFilter("educationLevel", event.target.value)
                    }
        ...
        {isLoading ? (
          ...
          <div
            className="rounded-card bg-surface py-16 text-center shadow-card"
            role="alert"
          >
